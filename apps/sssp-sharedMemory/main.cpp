...
int main(int argc, char** argv) {
  ...
  G.read(input_file);  
 // G.printGraph();
  uint64_t numbytes_dist = sizeof(unsigned) * G.h_nnodes;
  ...
  // finding the clustering coefficient of the nodes
  double *d_cc, *h_cc; // making 'dist' unsigned because atomicMin() is not supported on uint64_t
  uint64_t numbytes_cc = sizeof(double) * G.h_nnodes;
  h_cc = (double*)malloc(numbytes_cc);
  gpuErrchk(cudaMalloc(&d_cc, numbytes_cc));
  getClusteringCoeff(G, h_cc, d_cc, num_blocks, block_size); 
